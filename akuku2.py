...
# nazywa si jak [path_plik_nmap_msfconsole] 
    ...
    for linijka in otwarty_plik_nmap:
        ...
        if r.match(ip) is None:
            ...
            if(port == "21" or "FTP" in opis_nmap or "ftp" in opis_nmap or "Ftp" in opis_nmap):
                ...
                data['host'].append({ip:{'ftp':{'Dodatkowo&nbsp;mo|na:':f'<p style="color:red">{zalecenia_ftp}</p>\n'}}})
            ...
            if(port == "22" or "ssh" in opis_nmap or "SSH" in opis_nmap or "Ssh" in opis_nmap):
                ...
                data['host'].append({ip:{'ssh':{'Dodatkowo&nbsp;mo|na':f'<p style="color:red">{zalecenia_ssh}</p>\n'}}})

            # port 23
            # telnet
            zalecenia_telnet = f"nmap (NSE) <i><b>nmap --script telnet* -p23 -d {ip}</b></i>"
            if(port == "23"):
                data['host'].append({ip:{'telnet':{'Dodatkowo&nbsp;mo|na':f'<p style="color:red">{zalecenia_telnet}</p>\n'}}})
...
def f_get_links_from_web(ip,port,protokol,h_prot):
    ...
    if(h_prot == "http"):
        try:
            f_zapis_log("f_get_links_from_web", "http","info")
            ...
            #spis_linkow,spis_linkow_html = "error f_get_links_from_web http " + addrHTTP
    elif(h_prot == "https"):
        try:
            f_zapis_log("f_get_links_from_web", "https","info")
            ctx = ssl.create_default_context()
            ctx.check_hostname = False
            ctx.verify_mode = ssl.CERT_NONE
...
if __name__ == '__main__':
    ...
    if(str(args.fin) == '' or str(args.fin) == 'None'):
        path_plik_nmap_msfconsole = '/home/user/PWPW3'
    else:
