    ...
    for linijka in otwarty_plik_nmap:
        ...
        if r.match(ip) is None:
                ...
                if(output_curl != "none"):
                    ...
                    if(http_code == "000" or http_code == "200" or http_code == "301" or http_code == "302" or http_code == "404"):
                        try:
                            ...
                            if(http_code == "301"):
                                # wynik funkcji, ktory sprawdza na jaki adres
                                # przekierowuje
                                adres = f_curl_variables(adres, "redirect_url")
                                # zapis do logu
                                f_zapis_log(
                                    "f_odczyt_pliku_nmap", "info", f"adres [{adres}]")
                        ...
                        tmp_dict[ip][
                            'wskazowka:crawl:wget'] = f'wget --wait=2 --level=inf --limit-rate=20K --recursive --page-requisites --user-agent=Mozilla --no-parent --convert-links --adjust-extension --no-clobber -e robots=off {h_prot}://{ip}:{port} --no-check-certificate'
            #
            ...
            if(port == "22" or "ssh" in opis_nmap.lower()):
                ...
                tmp_dict[ip][
                    'wskazowka:[Brute-force]:patator'] = f"patator ssh_login host={ip} user=FILE0 0=logins.txt password=$(perl -e ""print 'A'x50000"") --max-retries 0 --timeout 10 -x ignore:time=0-3"
            ...
            if(port == "25" or "smtp" in opis_nmap.lower()):
                ...
                tmp_dict[ip][
                    'wskazowka:enum1'] = f"smtp-user-enum -M VRFY -U users.txt -t {ip}"
                tmp_dict[ip]['wskazowka:enum2'] = f"smtp-user-enum -M EXPN -u admin1 -t {ip}"
                tmp_dict[ip][
                    'wskazowka:enum3'] = f"smtp-user-enum.pl -M RCPT -U users.txt -T mail-server-ips.txt {ip}"
                tmp_dict[ip][
                    'wskazowka:enum4'] = f"smtp-user-enum.pl -M EXPN -D example.com -U users.txt -t {ip}"
...
def f_rpc_p135(ip):
    ...
    f_zapis_log(
        "f_rpc_p135",
        "info",
        f"[*] Wykryte adresy sieciowe hosta[{target_ip}]")
