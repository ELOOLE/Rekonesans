...
def f_make_thread(function,*args):
    """
    This decorator makes a thread out of the given function
    :param function: Function to be made into a thread
    :param args: all variables
    :return: 
    """
    my_thread1 = threading.Thread(target=function, args=args)
    my_thread1.start()
    my_thread1.join()
    return
...
def f_make_index(data_file, results_path):
    """
    Make index.html in directory where is data file. 
    File will contain links to results files from scan.

    :param data_file: Path of file with data, addresses to scan.
    :param results_path: Path where results are store.
    :return: 
    """
...
@f_results
def f_get_links_from_web(ip, port, proto, usluga, opis_nmap, results_path):
    ...
    try:
        ...
        save_results_in_file(spis_linkow_html, ip, port, proto, usluga, opis_nmap, results_path, "grep links from web")
        
        return spis_linkow_html
    except Exception as e:
        spis_linkow = "error"
        spis_linkow_html = "error"
        save_results_in_file("[-] fail of grab links from web", ip, port, proto, usluga, opis_nmap, results_path, "grep links from web")
        
        return "error"


@f_results
def f_screen_shot_web(ip, port, proto, usluga, opis_nmap, results_path):
    ...
    try:
        ...
        nazwa_pliku = f"{ip}_{port}_{proto}.png"

        # tresc znaku wodnego nanoszonego na *.png
        znak_wodny = f"{f_czas()} | Protokol: [{proto}], adres ip: [{ip}], port: [{port}]"

        save_pic_to_file = f"{results_path}/{nazwa_pliku}"
        ...
        save_results_in_file(pic_file_img_html, ip, port, proto, usluga, opis_nmap, results_path, "webshot")
        pic_file = f'<a href="{results_path}/{ip}_{port}_{proto}.jpg">{results_path}/{ip}_{port}_{proto}.jpg</a>'
        save_results_in_file(pic_file, ip, port, proto, usluga, opis_nmap, results_path, "webshot")
