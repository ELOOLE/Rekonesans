...
if __name__ == '__main__':
    ...
    parser.add_argument('-u', '--url', type=str, action='store', dest='url_addr', help='Podaj sciezke do pliku z adresami')
    parser.add_argument('-l', '--limit', type=int, action='store', dest='limit', help='Limit spr')
    ...
    for i in range(limit):
        ...
        h = def_header()
        req = http.post(url, timeout=TIME_OUT, verify=False, allow_redirects=True, headers=h, data='{"login":"admin", "password":"1234"}')
        raw = req.content.decode(encoding='utf-8', errors='ignore')

        if(raw in "Method Not Allowed"):
            #print(f"{raw} {h}" )
            print(style.RED(f"Respons: {raw}"), flush=True) 
            print(style.CYAN(f"Header: {h}"), flush=True)
        else:
            print(style.GREEN(f"Respons: {raw}"), flush=True) 
            print(style.CYAN(f"Header: {h}"), flush=True)
            sys.exit(1) 
