        ...
        if r.match(ip) is None:
            ...
            if(protokol.lower() == "tcp"):
                ...
                for h_prot in lista_protokol:
                    ...
                    # CURL %{url_effective} 
                    #cmd = "curl -skL -o /dev/null -w %{url_effective} " + adres
                    ...
                    # CURL %{http_code}
                    cmd = f'curl -A "{f_biblioteka.random_ua()}"' + "-skIL -s -o /dev/null -w \"%{http_code}\" " + adres
                    curl_output = f_biblioteka.f_polecenie_uniwersalne(cmd)
                    http_code = "000"
                    if(curl_output[1] == None):
                    ...
                    if(http_code in lista_http_code):
                        cmd = f"curl -skIL {h_prot}://{ip}:{port} --max-time 5 --no-keepalive"
                        curl_output = f_biblioteka.f_polecenie_uniwersalne(cmd)
                        if(curl_output[1] == None):
